...
def fit_ND():
    ...
    VyPy.sbo.tools.plot_spider_trace(ax,f1,x0,xb,20,'b-',lw=2,label='Fit')
    VyPy.sbo.tools.plot_spider_trace(ax,f2,x0,xb,20,'r-',lw=2,label='Truth')
...
def Rosenbrock_Function(X):
    X = VyPy.sbo.tools.check_array(X)
    D = X.shape[1]
    Y = 0.
    DY = X*0.
    for I in range(D):
    ...
    Y = VyPy.sbo.tools.check_array(Y,'col')
    return Y,DY
...
def Hyperplane_Function(X):
    X = VyPy.sbo.tools.check_array(X) + 0.5
    N,D = X.shape
    C = np.array([ np.arange(D)+1. ])
    I = np.ones([N,D])
    Y  = np.dot( X , C.T  )
    DY = C * I
    Y = VyPy.sbosbo.tools.check_array(Y,'col')
    return Y,DY
